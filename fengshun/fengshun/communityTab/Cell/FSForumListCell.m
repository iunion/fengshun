...
- (void)showWithFSCommunityForumListModel:(FSForumModel *)aModel
{
    ...
    // 已关注section
    self.m_DoBtn.userInteractionEnabled = !aModel.m_isAttentionSection;
    if (aModel.m_isAttentionSection)
    {
        self.m_DoBtn.backgroundColor = [UIColor bm_colorWithHex:0x4E7CF6];
        [self.m_DoBtn setTitleColor:[UIColor bm_colorWithHex: 0xffffff] forState:UIControlStateNormal];
        [self.m_DoBtn setTitle:@"查看" forState:UIControlStateNormal];
    }
    else
    {
...
- (IBAction)doFollowAction:(UIButton *)sender
{
    ...
    [FSApiRequest updateFourmAttentionStateWithFourmId:self.m_ForumModel.m_Id followStatus:!state success:^(id  _Nullable responseObject) {
        weakSelf.m_ForumModel.m_AttentionFlag = !weakSelf.m_ForumModel.m_AttentionFlag;
        [weakSelf showWithFSCommunityForumListModel:weakSelf.m_ForumModel];
        if (weakSelf.attentionBtnClickBlock) {
            weakSelf.attentionBtnClickBlock(weakSelf.m_ForumModel) ;
        }
