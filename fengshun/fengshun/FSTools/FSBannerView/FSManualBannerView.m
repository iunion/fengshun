...
#import "FSManualBannerView.h"
#import "UIImageView+WebCache.h"


@interface FSManualBannerView ()
<
...
@implementation FSManualBannerView

- (instancetype)initWithFrame:(CGRect)frame scrollDirection:(FSBannerViewScrollDirection)direction images:(NSArray *)images padding:(CGFloat)padding pageWidth:(CGFloat)pageWidth dataSource:(id <FSBannerViewDataSource>)adataSource
{
...
- (void)reloadBannerWithData:(NSArray *)images
{
    ...
    for (NSInteger i = 0; i < totalPage; i++)
    {
        ...
        if (self.dataSource)
        {
            ...
            addView.tag = FSBanner_StartTag+i;
        ...
        control.tag = FSBanner_CStartTag+i;
...
- (void)setCorner:(NSInteger)cornerRadius
{
    for (NSInteger i = 0; i < totalPage; i++)
    {
        UIView *view = (UIView *)[self.scrollView viewWithTag:FSBanner_StartTag+i];
        if (view)
        {
...
{
    NSInteger index = control.tag-FSBanner_CStartTag;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    for (NSInteger i = 0; i < totalPage; i++)
    {
        UIControl *control = (UIControl *)[self.scrollView viewWithTag:FSBanner_CStartTag+i];
        if (control)
        {
