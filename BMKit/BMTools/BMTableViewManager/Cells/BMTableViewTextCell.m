//
//  BMTableViewTextCell.m
//  BMTableViewManagerSample
//
//  Created by DennisDeng on 2017/8/22.
//  Copyright © 2017年 DennisDeng. DennisDeng rights reserved.
//
...
@implementation BMInsetTextField
...
@interface BMTableViewTextCell ()

@property (nonatomic, assign) BOOL enabled;
@property (nonatomic, assign) BOOL editable;

@property (nonatomic, strong) BMTextItem *item;
@property (nonatomic, strong) BMInsetTextField *textField;

@end

@implementation BMTableViewTextCell
@synthesize item = _item;

+ (BOOL)canFocusWithItem:(BMTextItem *)item
{
    if (item.enabled)
    {
...
- (void)cellDidLoad
{
    ...
    self.textField = [[BMInsetTextField alloc] initWithFrame:CGRectZero];
...
- (void)cellWillAppear
{
    ...
    {
        self.textField.textColor = [UIColor bm_colorWithHex:0x333333];
    }
    self.textField.placeholder = self.item.placeholder;
    if (self.item.textFieldPlaceholderColor)
    {
        [self.textField bm_setPlaceholderColor:self.item.textFieldPlaceholderColor];
    }
...
- (void)layoutDetailView:(UIView *)view minimumWidth:(CGFloat)minimumWidth
{
    ...
    {
        self.textField.bm_width = self.item.accessoryView.bm_left - 10 - self.textField.bm_left;
    }
}
...
{
    if ([object isKindOfClass:[BMTextItem class]] && [keyPath isEqualToString:@"enabled"])
    {
        BOOL newValue = [[change objectForKey: NSKeyValueChangeNewKey] boolValue];
        
        self.enabled = newValue;
    }
    else if ([object isKindOfClass:[BMTextItem class]] && [keyPath isEqualToString:@"editable"])
    {
        BOOL newValue = [[change objectForKey: NSKeyValueChangeNewKey] boolValue];
