...
@implementation UIFont (BMCategory)

+ (NSDictionary *)bm_allFamilyAndFonts
{
    NSArray *fontFamilies = (NSArray *)[UIFont familyNames];
    fontFamilies = [fontFamilies bm_sortArrayByKey:@"" ascending:YES];
...
+ (NSArray *)bm_fontsNameForFamilyName:(BMFamilyFontName)familyFontName
{
    NSArray *fontNames;
...
+ (NSString *)bm_fontNameToString:(BMFontName)fontName
{
    switch (fontName)
    {
#pragma clang diagnostic push
...
+ (UIFont *)bm_fontForFontName:(BMFontName)fontName size:(CGFloat)fontSize
{
    NSString *fontStrName = [UIFont bm_fontNameToString:fontName];
    if (fontStrName)
    {
